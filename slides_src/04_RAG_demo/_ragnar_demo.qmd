...
## 一個完整的 RAG 系統架構圖如下： {.small}

::: {style="overflow-y: scroll; height: 830px;"}
```{mermaid}
flowchart TD
  %% User / Client
...
抓取 Quarto 官方網站的 GitHub repo 內容，呼叫 quarto render 輸出網站的 HTML 內容，然後底層使用 [微軟的 Python markitdown 套件](https://github.com/microsoft/markitdown)將 HTML 轉換為 Markdown 內容，最後將這些 Markdown 內容切片，建立 [duckdb](https://duckdb.org/) 向量資料庫並匯入時加入實際網址對應。
...
此 R script 檔會使用上一個 R script 建立的向量資料庫，並且串接 OpenAI REST API 相容的 LLM model 來建立一個查詢 Quarto 官方網站內容的 RAG 查詢物件。
